...
final class ServiceRecognitionTests: XCTestCase {
    func testRecognizesDeveloperRuntimesZrokMinikubeAndSystemServices() throws {
        ...
        let systemService = try XCTUnwrap(byPort[10005])
        XCTAssertEqual(systemService.group, "Системные сервисы")
        XCTAssertFalse(systemService.defaultForwardingEnabled)
        XCTAssertFalse(systemService.isForwardingEnabled)
